...
citation:
  - type: "Dataset"
    name: "MIMIC-IV, a freely accessible electronic health record dataset (PhysioNet / MIT-LCP)"
    url: "https://physionet.org/content/mimiciv/"
  - type: "CreativeWork"
    name: "SAIL Databank — the Secure Anonymised Information Linkage Databank (Swansea University)"
    url: "https://saildatabank.com/"
faq:
  - q: "What is \"anonymisation by architecture\"?"
    a: "It is a design where a patient's identity is structurally separated from their clinical content, so re-identification is not blocked by a policy or a rule — it is impossible by construction. The secret is not kept; it is not present. Identifiers live in a separate vault with no join path to the clinical data."
  - q: "How is the Yajur Health Vault different from access control or simply removing names?"
    a: "Access control still keeps identity reachable to anyone who gets past the control, and 'removing names' leaves indirect identifiers and reversible tokens behind. The Vault splits identity from clinical data into physically separate stores with no join key, so even a full breach of the clinical store yields no identities."
  - q: "Why are patient identifiers random and never hashed?"
    a: "A hash of a short, structured value like a hospital MRN is reversible by brute force — an attacker can hash every plausible MRN and build a reverse lookup in seconds. A random surrogate drawn from a look-up table has no mathematical relationship to the original identifier, so there is nothing to reverse."
  - q: "Can the data ever reach an external AI model?"
    a: "No. 'No egress' is enforced structurally, not by policy: inference runs locally or on-premise, and there is no network path by which clinical data can leave the boundary to reach an external model or API."
  - q: "How does legitimate re-identification (break-glass) work?"
    a: "Through a governed, two-factor break-glass process against the separately-secured identity vault, where every re-identification is logged and auditable. Re-identification remains possible for genuine clinical need, but it is deliberate, rare, and fully traceable rather than a routine capability."
  - q: "Is the Health Vault approach defensible under India's DPDP Act and ABDM?"
    a: "Yes. The architecture aligns with the Digital Personal Data Protection Act 2023, ABDM's Health Data Management Policy, and international benchmarks such as HIPAA Safe Harbor and Expert Determination, the EU's 2025 anonymisation judgment, and the UK's direction. The paper is explicit that governance — the Five Safes and a motivated-intruder test — is the control, not the algorithm alone."
---
